    ...
    <div class="graph-container" ref="graphContainer">
      ...
      <div v-if="graphData" class="graph-view">
        ...
        <div v-if="showSimulationFinishedHint" class="graph-building-hint finished-hint">
          ...
          <span class="hint-text">Some content is still processing. Try refreshing the graph manually later.</span>
...
const renderGraph = () => {
  ...
  tempEdges.forEach(e => {
    ...
    // Calculate curvature: spread out for multiple edges, straight line for single
    let curvature = 0
    if (totalCount > 1) {
      ...
      // This ensures all edges are distributed in the same reference frame so they won't overlap due to different directions
      if (isReversed) {
        curvature = -curvature
      }
    }
  ...
  const simulation = d3.forceSimulation(nodes)
    ...
    // Add center gravity to cluster independent nodes
  ...
  const getLinkPath = (d) => {
    ...
    // Perpendicular offset based on distance ratio, ensures visible curves
