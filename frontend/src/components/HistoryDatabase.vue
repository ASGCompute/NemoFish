        ...
        <div v-if="selectedProject" class="modal-overlay" @click.self="closeModal">
          <div class="modal-content">
            ...
            <div class="modal-playback-hint">
              <span class="hint-text">Step 3 "Run Simulation" and Step 5 "Deep Interaction" must be launched during runtime. History replay is not supported.</span>
            </div>
          </div>
        </div>
      </Transition>
...
const getCardStyle = (index) => {
  ...
  if (isExpanded.value) {
    ...
    // Calculate cards per row, ensure centering
    ...
    const x = offset * 35
    // Adjust start position, close to title with appropriate spacing
  ...
  observer = new IntersectionObserver(
    (entries) => {
      entries.forEach((entry) => {
        ...
        // If target state matches current, no processing needed
        if (shouldExpand === isExpanded.value) {
          pendingState = null
          return
        }
        
        // Use debounce to delay state switch, prevent fast flicker
        ...
        expandDebounceTimer = setTimeout(() => {
          ...
          // Check if pending state still needs execution (may be overridden)
          ...
          setTimeout(() => {
            ...
            if (pendingState !== null && pendingState !== isExpanded.value) {
              // Brief delay before executing, avoid too-fast switching
              expandDebounceTimer = setTimeout(() => {
                if (pendingState !== null && pendingState !== isExpanded.value) {
                  isAnimating = true
                  isExpanded.value = pendingState
    ...
    },
    {
      ...
      // Adjust rootMargin, shrink viewport bottom upward, require more scroll to trigger expand
...
// If using keep-alive, reload on component activation
onActivated(() => {
  loadHistory()
})
...
.grid-pattern {
  ...
  /* Position from top-left, height changes expand from bottom only */
