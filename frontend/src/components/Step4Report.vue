            ...
            >
              ...
              <div class="timeline-content">
                ...
                <div class="timeline-body" :class="{ 'collapsed': isLogCollapsed(log) }" @click="toggleLogExpand(log)">
                  ...
                  <!-- Section Content Generated (Content generated, but entire section may not be done) -->
...
const parseInsightForge = (text) => {
  ...
  try {
    // 提取分析Question
    const queryMatch = text.match(/分析Question:\s*(.+?)(?:\n|$)/)
    ...
    // Extract statistics - 匹配"相关预测事实: Xitems"格式
    ...
    // Extract core entities - full extraction with summaries and related facts count
    ...
    // Extract relationship chains - full extraction, unlimited
...
const parsePanorama = (text) => {
  ...
  try {
    ...
    // Extract active facts - full extraction, unlimited
    ...
    const parseIndividualReasons = (reasonText) => {
      ...
      for (const line of lines) {
        ...
        if (name) {
          ...
          // Reason continuation (excluding final summary paragraphs)
    ...
    interviewBlocks.forEach((block, index) => {
      ...
      if (answerMatch) {
        ...
        if (!twitterMatch && redditMatch) {
          // Only Reddit answer, copy as default when not placeholder
          ...
          // No platform markers (legacy format), use entire as answer
      ...
      if (quotesMatch) {
        ...
        // Prioritize > "text" format
        let quoteMatches = quotesText.match(/> "([^"]+)"/g)
        // 回退：匹配 > "text" 或 > \u201Ctext\u201D（Chinese quotes)
        if (!quoteMatches) {
  ...
  setup(props) {
    ...
    return () => h('div', { class: 'insight-display' }, [
      ...
      h('div', { class: 'insight-content' }, [
        ...
        // Empty state
        activeTab.value === 'facts' && props.result.facts.length === 0 && h('div', { class: 'empty-state' }, '暂无Current Key Memories'),
        activeTab.value === 'entities' && props.result.entities.length === 0 && h('div', { class: 'empty-state' }, '暂无Core Entities'),
        activeTab.value === 'relations' && props.result.relations.length === 0 && h('div', { class: 'empty-state' }, '暂无Relationship Chains')
      ])
    ])
  }
}
    ...
    return () => h('div', { class: 'panorama-display' }, [
      ...
      h('div', { class: 'panorama-content' }, [
        ...
        activeTab.value === 'active' && h('div', { class: 'facts-panel active-facts' }, [
          ...
          ) : h('div', { class: 'empty-state' }, '暂无Active Memories'),
        ...
        activeTab.value === 'historical' && h('div', { class: 'facts-panel historical-facts' }, [
          ...
          ) : h('div', { class: 'empty-state' }, '暂无Historical Memories'),
        ...
        activeTab.value === 'entities' && h('div', { class: 'entities-panel' }, [
          ...
          ) : h('div', { class: 'empty-state' }, '暂无Involved Entities'),
...
const InterviewDisplay = {
  ...
  setup(props) {
    ...
    // 检查是否为platform placeholder text
    const isPlaceholderText = (text) => {
      if (!text) return true
      const t = text.trim()
      return t === '（No response from this platform）' || t === '(No response from this platform)' || t === '[No reply]'
    }
    ...
    const splitAnswerByQuestions = (answerText, questionCount) => {
      ...
      // 1. "QuestionX:" (Chinese format, backend new format) （Chinese format, backend new format）
      ...
      // If no match, fall back to "数字." 格式
      if (matches.length === 0) {
        const numPattern = /(?:^|[\r\n]+)(\d+)\.\s+/g
    ...
    const getAnswerForQuestion = (interview, qIdx, platform) => {
      ...
      // 分割失败：第一个Question返回完整回答，其余返回空
      return qIdx === 0 ? answer : ''
    }
    
    // 检查某个Question是否有双平台回答（过滤占位文本）
              ...
              answerText && h('div', { class: ['qa-answer', { 'answer-placeholder': isPlaceholder }] }, [
                ...
                h('div', { class: 'qa-content' }, [
                  h('div', { class: 'qa-answer-header' }, [
                    ...
                    // Dual-platform switch buttons (shown only when both platforms have answers)
...
const renderMarkdown = (content) => {
  ...
  // Remove leading ## headings since section titles are shown in outer layer
  ...
  // Clean <p><br> followed by block elements (from extra empty lines)
      ...
      if (newLogs.length > 0) {
        newLogs.forEach(log => {
          ...
          if (log.action === 'report_complete') {
            ...
            currentSectionIndex.value = null  // Clear loading state
        ...
        nextTick(() => {
          if (rightPanel.value) {
            // If task done, scroll to top; otherwise scroll to bottom to follow latest logs
            if (isComplete.value) {
...
const extractFinalContent = (response) => {
  ...
  // Try extracting content from <final_answer> tags
  ...
  // Try finding content after Final Answer in Chinese
