...
Converts entities from Zep graph into Agent Profile format for the OASIS simulation platform.
...
logger = get_logger('nemofish.oasis_profile')
...
class OasisProfileGenerator:
    ...
    3. Distinguishes between individual and abstract group entities
    """
    ...
    # Group/org entity types (need representative persona)
    GROUP_ENTITY_TYPES = [
    ...
    def _search_zep_for_entity(self, entity: EntityNode) -> Dict[str, Any]:
        ...
        Zep has no built-in hybrid search API, so we search edges and nodes separately
        and merge results. Uses parallel requests for efficiency.
    ...
    def _build_entity_context(self, entity: EntityNode) -> str:
        ...
        Includes:
        1. Edge information (facts) about the entity itself
    ...
    def _is_individual_entity(self, entity_type: str) -> bool:
        """Check if entity is an individual type"""
        return entity_type.lower() in self.INDIVIDUAL_ENTITY_TYPES
    
    def _is_group_entity(self, entity_type: str) -> bool:
        """Check if entity is a group/institutional type"""
    ...
    def _generate_profile_with_llm(
        ...
        """
        Generate highly detailed persona using LLM.
        
        Distinguished by entity type:
        - Individual entity: generate specific character profile
        - Group/org entity: generate representative account profile
        """
        ...
        for attempt in range(max_attempts):
            try:
                response = self.client.chat.completions.create(
                    ...
                    # No max_tokens limit, let LLM generate freely
                )
        ...
        logger.warning(f"LLM persona generation failed ({max_attempts} attempts): {last_error}, using rule-based fallback")
    ...
    def _get_system_prompt(self, is_individual: bool) -> str:
        """Get system prompt"""
        base_prompt = "You are a social media user profile generation expert. Generate detailed, realistic personas for opinion simulation, maximally reflecting known real-world facts. You must return valid JSON format, and all string values must not contain unescaped newlines. Use English."
        return base_prompt
    ...
    def _build_individual_persona_prompt(
        ...
        return f"""Generate a detailed social media user persona for this entity, maximally reflecting known real-world facts.
...
1. bio: Social media bio, approximately 200 words
2. persona: Detailed persona description (~2000 words, plain text), including:
   ...
   - Positions and opinions (stance on topics, content that may provoke or move them)
   - Unique characteristics (catchphrases, special experiences, personal hobbies)
   - Personal memory (critical part: describe how this individual relates to the events, and their past actions and reactions in those events)
3. age: Age as a number (must be integer)
4. gender: Must be English: "male" or "female"
5. mbti: MBTI type (e.g. INTJ, ENFP)
6. country: Country name in English (e.g. "United States", "China")
...
"""
    ...
    def _build_group_persona_prompt(
        ...
        """Build detailed persona prompt for group/org entity"""
        
        attrs_str = json.dumps(entity_attributes, ensure_ascii=False) if entity_attributes else "None"
        context_str = context[:3000] if context else "No additional context"
        
        return f"""Generate a detailed social media account profile for this institutional/group entity, maximally reflecting known real-world facts.
...
1. bio: Official account bio, approximately 200 words, professional and appropriate
2. persona: Detailed account profile description (~2000 words, plain text), including:
   - Organization basics (official name, nature, founding context, main functions)
   - Account positioning (account type, target audience, core functions)
   - Communication style (language patterns, common expressions, taboo topics)
   - Content characteristics (content types, posting frequency, active time periods)
   - Official stance (position on core topics, approach to controversies)
   - Special notes (represented group demographics, operational habits)
   - Institutional memory (critical part: describe how this institution relates to events, and its past actions and reactions)
3. age: Always 30 (virtual age for institutional accounts)
4. gender: Always "other" (institutional accounts use "other" to indicate non-personal)
...
- persona must be a coherent paragraph, do not use newline characters
- Use English for ALL fields
...
- Institutional account communications must match its identity and positioning"""
    ...
    def _generate_profile_rule_based(
        ...
        # Generate different persona based on entity type
    ...
    def generate_profiles_from_entities(
        ...
        Args:
            ...
            graph_id: Graph ID for Zep retrieval
            parallel_count: Parallel generation count, default 5
            realtime_output_path: Path for real-time output (writes after each generation)
        ...
        # Helper function for real-time file saving
        def save_profiles_realtime():
            ...
            with lock:
                # Filter already generated profiles
        ...
        with concurrent.futures.ThreadPoolExecutor(max_workers=parallel_count) as executor:
            ...
            for future in concurrent.futures.as_completed(future_to_entity):
                ...
                try:
                    ...
                    # Write file in real-time（即使是备用Persona）
                    save_profiles_realtime()
    ...
    def save_profiles(
        ...
        """
        Save profiles to file (choosing correct format per platform).
    ...
    def _save_twitter_csv(self, profiles: List[OasisAgentProfile], file_path: str):
        """
        Save Twitter Profile as CSV format (per OASIS requirements).
        
        OASIS Twitter required CSV fields:
        - user_id: User ID (sequential from 0)
        - name: Real name
        - username: System username
        - user_char: Detailed persona (injected into LLM system prompt, guides agent behavior)
        - description: Short public bio (displayed on profile page)
        
        user_char vs description:
        - user_char: Internal use, LLM system prompt, determines how agent thinks/acts
